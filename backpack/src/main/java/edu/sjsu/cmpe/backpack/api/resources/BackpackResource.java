...
import freemarker.template.TemplateModelException;
	...
	public class BackpackResource {
		...
		private final String subsQueueName;
		
	    public BackpackResource(Connection connection, String queueName, String subsQueueName) {
	    	try {
	    	...
	    	this.subsQueueName=subsQueueName;
...
//    	ob.append("filesShared",new ArrayList<String>());   	
    	...
    	try {
			sendMessage(subsQueueName, user.getEmail(), null, user.getFirstName()+" "+user.getLastName(), null, null);
...
//			root.put("accessType", newFile.getAccessType());
	...
	public Response updateFileByEmail(@Context HttpServletRequest req,@PathParam("userID") int userID, @PathParam("id") int fileID,
		...
		if (person != null){
			...
			String link = urlInit+":8000/backpack/v1/users/"+personID+"/filesShared/"+fileID;
//			sendEmail(sharedWithID, fileName,ownerFullName, recieverFullName, link);
			try {
				sendMessage(queueName,sharedWithID, fileName,ownerFullName, recieverFullName, link);
			} catch (JMSException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
...
*/    
    ...
    public void sendMessage(String queue,String Email, String fileName, String owner, String reciever, String link) throws JMSException{
//    	String queue = "/queue/59640.book.orders";
		...
		String data = Email+";"+fileName+";"+owner+";"+reciever+";"+link; 
